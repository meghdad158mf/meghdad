...
2. قبل از مارکر `// ---- boot ----`، بوت اصلی رو با این جایگزین کن (نه با صداکردن مستقیم `enterDash` — باید `loadAll()` هم صدا زده بشه). ⚠️ **این لیست باید کامل باشه** — `loadAll()` همه‌ی این توابع رو صدا می‌زنه و اگه حتی یکی override نشده باشه، fetch واقعی به Supabase (که این محیط بهش دسترسی نداره) fail می‌شه و چون `loadAll().then(...)` بدون `.catch()`ه، کل promise chain reject می‌شه و `enterDash` هیچ‌وقت صدا زده نمی‌شه — نتیجه: صفحه‌ی ورود (login) به‌جای داشبورد نشون داده می‌شه، بدون هیچ خطای واضحی (این باگ واقعاً یه‌بار رخ داد و کلی وقت گرفت تا پیدا بشه):
   ...
   loadDossierTopics = async function(){ DOSSIER_TOPICS = []; };   // اگه تب «پرونده ویژه» رو تست می‌کنی حتماً پر کن
   loadDossierTopicPosts = async function(){ DOSSIER_TOPIC_POSTS = []; };
...
GitHub Actions (کالکتورها با cron دوره‌ای، پاک‌سازی روزانه، تحلیل هوش مصنوعی روزی ۴بار — فقط از شاخه‌ی main اجرا می‌شن، نگاه کن به نکته‌ی عملیاتی پایین)
    ...
    └── scripts/extract_keywords.py  -- هر ۲ ساعت (ساعت‌های فرد) Edge Function extract-post-keywords رو صدا می‌زنه برای استخراج کلیدواژه‌ی خودکار پست‌ها، پایه‌ی تب «پرونده ویژه» (بخش «هوش مصنوعی — کلیدواژه‌ی خودکار پست‌ها» پایین‌تر رو ببین)
    │
    ▼
...
Supabase Edge Functions (supabase/functions/translate, supabase/functions/news-insights, supabase/functions/extract-post-keywords) ── درگاه هوش مصنوعی لیارا
```

هیچ بک‌اند سفارشی سنتی (FastAPI و مشابه) وجود نداره — فرانت‌اند مستقیم با Supabase حرف می‌زنه؛ تنها استثنا سه Edge Function سبک (Deno، سمت Supabase) برای هوش مصنوعیه (ترجمه + تحلیل خودکار اخبار + استخراج کلیدواژه‌ی پست‌ها)، چون کلید هوش مصنوعی نمی‌تونه توی فرانت‌اند هاردکد بشه (بخش‌های «هوش مصنوعی» پایین رو ببین).
...
⚠️ **تاریخچه‌ی زمان‌بندی**: اول ۲ بار در روز (هر ۱۲ ساعت، `۰۸:۰۰`/`۲۰:۰۰`) بود؛ طبق درخواست صریح کاربر به ۴ بار در روز (هر ۶ ساعت) تغییر کرد. **درسِ گرفته‌شده‌ی عمومی‌تر** (نکته‌ی عملیاتی ۲۰ پایین‌تر رو هم ببین، اصلاح‌شده): وقتی این کرون جدید فقط ~۳۰ دقیقه قبل از اولین اسلاتش (۰۲:۰۰ ایران) روی `main` merge شد، اون اجرا سر وقت (۲۲:۳۰ UTC) توی `list_workflow_runs` دیده نشد و اولش فکر کردیم GitHub اون اسلات رو کامل skip کرده. ولی با پیگیری بعدی (چک `list_workflow_runs` دوباره، یکی‌دو ساعت بعد) معلوم شد **skip نشده بود، فقط با ~۱.۵ ساعت تأخیر واقعی اجرا شده** (`event="schedule"`، زمان واقعی اجرا ۰۰:۰۷ UTC به‌جای ۲۲:۳۰ UTC) و کامل موفق بود (۱۰ خبر منتخب + ۶ موضوع). یعنی نتیجه‌گیری اولیه («merge نزدیک به زمان اجرا باعث skip می‌شه») عجولانه بود — GitHub برای `schedule` trigger گاهی تأخیر قابل‌توجه (حتی بیش از ۱ ساعت) داره ولی معمولاً بالأخره اجرا می‌شه، نه اینکه کامل حذفش کنه. اگه یه اسلات توی `list_workflow_runs` بلافاصله بعد از زمان اسمی‌ش دیده نشد، قبل از نتیجه‌گیری «skip شده»، حداقل یکی‌دو ساعت صبر کن و دوباره چک کن.
...
## هوش مصنوعی — کلیدواژه‌ی خودکار پست‌ها (سومین Edge Function، پایه‌ی تب «پرونده ویژه»)

برخلاف دو فیچر بالاتر که خروجی‌شون مستقیماً به کاربر نمایش داده می‌شه، این یکی یه **مرحله‌ی پیش‌پردازش/زیرساختیه**: هر پست فقط یک‌بار به چندتا کلیدواژه‌ی فارسی برچسب می‌خوره تا بعداً بخش «پرونده ویژه» (بالاتر ببین) بتونه موضوعات موقت/چرخشی رو بدون هزینه‌ی هوش مصنوعی اضافه، باهاش تطبیق بده.

**معماری**:
```
GitHub Actions (.github/workflows/extract-keywords.yml، هر ۲ ساعت — 30 1-23/2 * * *)
    │
    ├── scripts/extract_keywords.py -- با public.login() به‌عنوان app_admin وارد می‌شه (مثل بقیه)
    ▼
Edge Function supabase/functions/extract-post-keywords (با توکن مدیر صدا زده می‌شه)
    │
    ├── fetchPostsMissingKeywords() (تابع جدید در _shared/auth.ts) -- قدیمی‌ترین پست‌های
    │   posts.ai_keywords IS NULL (ORDER BY posted_at ASC، همون منبع show_in_news که
    │   news-insights هم استفاده می‌کنه) رو با توکن کالر می‌خونه
    ├── فراخوانی لیارا AI (gpt-4o-mini) با این پست‌ها — استخراج کلیدواژه، نه انتخاب/تحلیل
    ├── اعتبارسنجی: idهای هذیان‌گفته‌شده حذف می‌شن؛ پستی که هوش مصنوعی جا انداخته هم
    │   صریح با آرایه‌ی خالی علامت می‌خوره (نه رها‌شدن با NULL)
    ├── PATCH روی posts.ai_keywords + ai_keywords_extracted_at برای کل دسته
    └── تطبیق خودکار موضوعات فعال dossier_topics با کلیدواژه‌های همین دسته‌ی تازه
        (مقایسه‌ی متنی ساده، نه AI) → درج ردیف‌های dossier_topic_posts (source='auto')
```

**زمان‌بندی عمداً مستقل از news-insights و از کالکتورها**: کالکتورهای اصلی (`collect-eitaa`/`collect-telegram`/`collect-rss`) هر ۲ ساعت زوج (دقیقه‌ی ۰۰/۱۵) پست تازه میارن؛ اگه استخراج کلیدواژه به کرون ۶ساعته‌ی `news-insights` گره می‌خورد، پست‌های تازه تا ۶ ساعت بدون کلیدواژه می‌موندن. برای همین این کرون **زمان‌بندی جدای خودش** رو داره: هر ۲ ساعت، ولی روی **ساعت‌های فرد** (`30 1-23/2 * * *`) — دقیقاً وسط فاصله‌ی دو اجرای متوالی کالکتورها، نه هم‌زمانشون.

**resumable با NULL/آرایه‌ی خالی، نه یه فلگ جدا**: `posts.ai_keywords`:
- `NULL` = هنوز بررسی نشده (کاندید اجرای بعدی — `fetchPostsMissingKeywords` همیشه قدیمی‌ترین این‌ها رو می‌گیره، پس اگه یه اجرا fail بشه یا نصفه بمونه، اجرای بعدی خودکار از همونجا ادامه می‌ده).
- آرایه‌ی خالی `{}` = بررسی شد ولی کلیدواژه‌ی معناداری نداشت — **عمداً از `NULL` جداست**، چون اگه پستی که هیچ‌وقت کلیدواژه‌ی مفیدی نمی‌گیره `NULL` می‌موند، هر اجرا دوباره و دوباره براش به هوش مصنوعی درخواست می‌زد (توکن الکی). به همین دلیل هم توی خودِ Edge Function، پستی که هوش مصنوعی جا انداخته صریح با آرایه‌ی خالی پر می‌شه، نه رها شدن با `NULL`.

**قوانین کیفیت کلیدواژه (توی prompt، طبق درخواست صریح کاربر)**:
- **همیشه فارسی**، صرف‌نظر از زبان اصلی پست (اسم/موضوع خارجی هم ترجمه بشه، نه دست‌نخورده رها بشه) — تا موضوعات `dossier_topics` (که فارسی تعریف می‌شن) با پست‌های هر زبانی یکسان تطبیق داده بشن.
- **تعداد متغیر، نه ثابت** — هرچند کلیدواژه لازمه که همه‌ی موضوعات/اشخاص/نهادهای مهم پست کامل پوشش داده بشه (می‌تونه ۳ تا باشه، می‌تونه ۹ تا)، نه یه عدد تحمیلی ثابت.
- **بدون ابهام** — یه اصطلاح عام مثل «وزارت خارجه» یا «رئیس‌جمهور» به‌تنهایی قابل‌قبول نیست اگه بیشتر از یه کشور/نهاد بهش اشاره داشته باشه؛ باید مشخص بشه («وزارت خارجه ایران» در برابر «وزارت خارجه آمریکا»).
- **هیچ پستی بدون پاسخ صریح نمونه** — طبق بند بالا (NULL در برابر آرایه‌ی خالی).

**بازبینی/اصلاح دستی**: چون خطای هوش مصنوعی توی این فیچر مستقیماً روی کیفیت «پرونده ویژه» اثر می‌ذاره، مدیر می‌تونه رندوم چک کنه — توی همون فیدهای «شبکه‌های اجتماعی»/«وب‌سایت‌ها» (جایی که همیشه اخبار رو مرور می‌کنه)، زیر هر پست چیپ کلیدواژه‌هاش دیده می‌شه با یه دکمه‌ی «ویرایش کلیدواژه» که مستقیم `posts.ai_keywords` رو patch می‌کنه. جزئیات کامل توی بخش «تب «پرونده ویژه»» بالاتر.

**نکات امنیتی**: همون الگوی دو فیچر بالاتر — فرانت‌اند/کالر فقط `limit` (اختیاری) می‌فرسته، نه متن آزاد؛ خودِ Edge Function پست‌های واقعی رو با توکن کالر می‌خونه.

**دیپلوی**: مثل بقیه، خودکاره — `deploy-edge-functions.yml` یه step جدا برای `extract-post-keywords` داره؛ همون secretهای موجود رو به اشتراک می‌ذاره.

⚠️ **sync با `main` لازمه**: `scripts/extract_keywords.py` و `.github/workflows/extract-keywords.yml` باید مثل بقیه‌ی کالکتورها با یه PR جدا به `main` sync بشن (خودِ Edge Function push-based دیپلوی می‌شه، نیازی به sync نداره).
...
db/migration_023_dossier.sql               posts.ai_keywords/ai_keywords_extracted_at + جدول‌های dossier_topics و dossier_topic_posts (تب «پرونده ویژه») — select هر دو نقش، insert/update/delete فقط مدیر
...
scripts/extract_keywords.py                هر ۲ ساعت (ساعت‌های فرد) Edge Function extract-post-keywords رو صدا می‌زنه — بخش «هوش مصنوعی — کلیدواژه‌ی خودکار پست‌ها» رو ببین
...
.github/workflows/extract-keywords.yml     هر ۲ ساعت، ساعت‌های فرد (30 1-23/2 * * *) — عمداً وسط فاصله‌ی کالکتورها، نگاه کن به بخش «هوش مصنوعی — کلیدواژه‌ی خودکار پست‌ها»
...
supabase/functions/extract-post-keywords/index.ts  Edge Function استخراج کلیدواژه‌ی هوش مصنوعی هر پست، پایه‌ی تب «پرونده ویژه» — بخش «هوش مصنوعی — کلیدواژه‌ی خودکار پست‌ها» رو ببین
...
- **posts**: id, channel_id, platform, platform_post_id, text, **title** (پست‌های RSS و بله پر می‌کنن)، media_type, **media_storage_path**، **media_source_url** (فقط ایتا)، **media_fetched_at**، posted_at, scraped_at, views, forwards, link — یکتا روی (channel_id, platform_post_id). **ai_keywords** (`text[]`، migration_023) — `NULL`=هنوز بررسی‌نشده (کاندید اجرای بعدی `extract-post-keywords`)، آرایه‌ی خالی=بررسی‌شده‌ی بی‌کلیدواژه؛ **ai_keywords_extracted_at** — پایه‌ی تب «پرونده ویژه»، بخش «هوش مصنوعی — کلیدواژه‌ی خودکار پست‌ها» رو ببین.
...
- **dossier_topics**: id, name, keywords (متن، جدا با ویرگول، مثل `news_topics`)، **active** (فقط موضوعات فعال به بیننده نشون داده می‌شن؛ غیرفعال‌کردن پست‌هاش رو حذف نمی‌کنه)، sort_order, created_at — موضوعات موقت/چرخشی تب «پرونده ویژه»؛ migration_023. مدیریت (افزودن/فعال‌سازی/حذف) از پنل «مدیریت ← تنظیمات»؛ select هر دو نقش، insert/update/delete فقط مدیر.
- **dossier_topic_posts**: id, topic_id (FK), post_id (FK), **source** (`auto`|`manual`)، **excluded** (پنهان‌کردن از بیننده بدون حذف کامل)، created_at — یکتا روی (topic_id, post_id)؛ migration_023. ردیف‌های `auto` هر ۲ ساعت خودکار توسط `extract-post-keywords` درج می‌شن (تطبیق متنی کلیدواژه)، ردیف‌های `manual` از پنل مدیریت (جست‌وجوی دستی). select هر دو نقش، insert/update/delete فقط مدیر.
...
## تب «پرونده ویژه» (داخل `sec-overview`، `dossier`)

⚠️ **تاریخچه**: این تب قبلاً یه placeholder کامل با نشان «به‌زودی» بود (هم توی `.news-page-tabs` هم توی میان‌بر صفحه‌ی نخست `.domain-shortcuts`). طبق درخواست کاربر، اخبار مرتبط با چند «موضوع خاص و موقت» (که ممکنه هر چند روز عوض بشه) رو جدا گروه‌بندی‌شده نشون می‌ده — منبعش همون پست‌های موجود تب‌های «شبکه‌های اجتماعی»/«وب‌سایت‌ها»ست، نه کالکتور/پلتفرم جدید.

- **دسترسی**: `setNewsPageTab('dossier')` — از تب بالای صفحه، یا میان‌بر «پرونده ویژه» توی `.domain-shortcuts` (`goToNewsTab('dossier')`).
- **معماری اصلی مبتنی بر پیش‌پردازش، نه هوش مصنوعی زنده**: چون موضوعات مکرراً عوض می‌شن، اگه بخوایم هر بار موضوعی تعریف/تغییر می‌کنه هوش مصنوعی کل تاریخچه‌ی پست‌ها رو دوباره بخونه، هزینه‌ی توکن خیلی بالا می‌ره. راه‌حل: هر پست فقط **یک‌بار** (وقتی جمع‌آوری می‌شه) با هوش مصنوعی به چندتا کلیدواژه‌ی فارسی/دقیق برچسب می‌خوره (`posts.ai_keywords`، بخش «هوش مصنوعی — کلیدواژه‌ی خودکار پست‌ها» پایین‌تر رو ببین)؛ تطبیق موضوع بعدش فقط یه مقایسه‌ی متنی سادهٔ رایگانه (نه فراخوانی جدید هوش مصنوعی).
- **جدول‌ها** (`migration_023_dossier.sql`):
  - `dossier_topics`: موضوعات موقت/چرخشی — `name`, `keywords` (جدا با ویرگول، مثل `news_topics`)، `active` (فقط موضوعات فعال به بیننده نشون داده می‌شن؛ غیرفعال‌کردن پست‌هاش رو حذف نمی‌کنه، برای استفاده‌ی دوباره‌ی بعدی)، `sort_order`.
  - `dossier_topic_posts`: پست‌های واقعاً منتسب به هر موضوع — `topic_id`, `post_id`, `source` (`auto`|`manual`)، `excluded` (مدیر بدون حذف کامل می‌تونه یه مورد خودکار رو از دید بیننده پنهان کنه)، `unique(topic_id, post_id)`.
- **تطبیق خودکار**: هر ۲ ساعت، بعد از استخراج کلیدواژه‌ی دسته‌ی تازه، Edge Function `extract-post-keywords` موضوعات `active` رو با کلیدواژه‌های همون دسته تطبیق می‌ده (شامل‌بودن رشته‌ای دوطرفه، `pk.includes(tk) || tk.includes(pk)`) و ردیف‌های `source='auto'` جدید درج می‌کنه.
- **نمایش به بیننده** (`renderDossierTab()`): فقط موضوعات `active=true` که حداقل یه `dossier_topic_posts` با `excluded=false` دارن؛ هرکدوم یه بلوک جدا (`.dossier-group`) با عنوان موضوع + کارت‌های خبر (`dossierPostCardHtml()`، مرتب‌شده بر اساس `posted_at` نزولی). ⚠️ کارت‌های این تب عمداً **بدون هیچ عنصر تعاملی با id مشترک** (نه دکمه‌ی «نمایش بیشتر»، نه دکمه‌ی ترجمه) ساخته می‌شن — چون این پنل هم‌زمان با فیدهای دیگه (`news-study-panel` و مشابه) توی DOM می‌مونه (فقط `display:none` می‌شه، نه حذف)، و اگه یه پست هم‌زمان توی فید اصلی و توی «پرونده ویژه» ظاهر بشه، id تکراری (`news-text-N` و مشابه) باعث تداخل می‌شد.
- **پنل مدیریت** (مدیریت ← تنظیمات ← «موضوعات پرونده ویژه»): افزودن موضوع (نام+کلیدواژه)، چک‌باکس فعال/غیرفعال هر موضوع، حذف کامل موضوع (پست‌های منتسبش هم `cascade` حذف می‌شن). برای هر موضوع، لیست `dossier_topic_posts`ش با دکمه‌ی «پنهان»/«نمایش» (toggle `excluded`، بدون حذف) و دکمه‌ی حذف کامل ردیف؛ + یه اینپوت جست‌وجوی دستی (`searchDossierManualAdd()`، تطبیق زنده روی عنوان/متن `POSTS_CACHE`) برای افزودن پستی که تطبیق خودکار کلیدواژه‌ای نگرفته (`source='manual'`).
- **بازبینی/اصلاح کلیدواژه‌ی هر پست، مستقل از این تب**: توی همون فیدهای «شبکه‌های اجتماعی»/«وب‌سایت‌ها» (نه فقط اینجا)، فقط برای مدیر، زیر هر پست یه ردیف چیپ کلیدواژه (`keywordChipsHtml()`) + دکمه‌ی «ویرایش کلیدواژه» (`editPostKeywords()`/`savePostKeywords()`) هست — برای اینکه مدیر بتونه رندوم چک/اصلاح کنه، بدون نیاز به صفحه‌ی جدا. جزئیات کامل توی بخش «هوش مصنوعی — کلیدواژه‌ی خودکار پست‌ها» پایین‌تر.
...
21. ⚠️⚠️ **رفع نهایی حادثه‌ی ۱۷ شهریور ۱۴۰۵ (نکته‌ی عملیاتی ۱۳) — ریشه‌ی واقعی یه باگ silent-fail توی حذف Storage بود، نه فقط عدم‌هماهنگی فرکانس کرون**: بعد از رفع عدم‌هماهنگی کرون (نکته‌ی ۱۳)، کاربر چندبار Storage رو دوباره چک کرد و عدد **هیچ تغییری نکرد** — یعنی جاب `cleanup-media.yml` هرروز گزارش موفقیت می‌داد («[done] cleaned up N post(s)») ولی چیزی واقعاً از Storage کم نمی‌شد. ریشه پیدا شد: `remove_storage_objects()` توی `scripts/cleanup_media.py` فقط status code پاسخ (`r.ok`) رو چک می‌کرد. **Supabase Storage bulk-delete endpoint (`DELETE /storage/v1/object/{bucket}` با `{"prefixes":[...]}`) حتی وقتی هیچ‌کدوم از prefixهای داده‌شده با فایل واقعی match نمی‌کنن، HTTP 200 با یه آرایه‌ی خالی برمی‌گردونه** — یعنی «صفر فایل حذف شد» ظاهراً همون status موفقِ «همه حذف شدن» رو داره. نتیجه: تابع همیشه `True` برمی‌گردوند، caller (`cleanup_post_media`/`cleanup_newspaper_covers`) هم بلافاصله `media_storage_path` دیتابیس رو پاک می‌کرد — یعنی رد دیتابیسی فایل از بین می‌رفت ولی خودِ فایل هیچ‌وقت واقعاً حذف نمی‌شد و برای همیشه orphan (بدون هیچ ردی) توی Storage می‌موند.
    ...
    - ⚠️ **کپی جدای همون باگ توی `scripts/purge_orphaned_media.py` هم بود**: بعد از رفع بالا و اجرای `purge-orphaned-media.yml` با `dry_run=false`، اسکریپت گزارش داد «۲۴۶/۲۴۶ فایل orphan حذف شد» — ولی این اسکریپت خودش یه کپی جدای دیگه از همون تابع `remove_storage_objects()` رو داشت (کدشون از اول جدا نوشته شده بود، نه import مشترک) که موقع رفع بالا از قلم افتاده بود و هنوز فقط status code رو چک می‌کرد. یعنی همون پیام «۲۴۶/۲۴۶» هم می‌تونست دروغ باشه. این هم جدا رفع شد (همون الگو: تعداد واقعی حذف‌شده از بدنه‌ی پاسخ). **درس**: وقتی یه تابع در دو فایل جدا copy-paste می‌شه، رفع باگ توی یکی خودکار اون یکی رو رفع نمی‌کنه — هر دو رو صریح چک کن.
    - ✅ **تأیید نهایی که رفع واقعاً جواب داد**: بعد از رفع کپی دوم، `purge-orphaned-media.yml` دوباره (این‌بار با تابع اصلاح‌شده‌ی هردو اسکریپت) اجرا شد و لیست تازه‌ی خودِ Storage API نشون داد `post-media` از ۵۲۵ فایل به ۲۷۹ فایل رسیده (دقیقاً ۲۴۶ تا کمتر) با **۰ orphan** — یعنی حذف قبلی هم واقعاً درست کار کرده بود، نه فقط ادعا.
    - ⚠️⚠️ **تله‌ی نهایی که کل این حادثه رو بزرگ‌تر از واقعیتش نشون داد: کاربر داشت SQL Editor یه پروژه‌ی کاملاً اشتباه (پروژه‌ی قدیمی و متروکه‌ی `tfppjveupcxisepteibn`، نکته‌ی عملیاتی ۱۸) رو چک می‌کرد، نه پروژه‌ی زنده‌ی فعلی (`komqnapfqrtxxaytpcdt`)**. برخلاف Storage/REST API پروژه‌ی قدیمی که کامل با ۴۰۲ مسدود شده، دسترسی SQL Editor بهش هنوز کار می‌کنه — پس کوئری واقعاً اجرا می‌شد و عدد واقعی برمی‌گردوند، فقط از یه دیتابیس کاملاً بی‌ربط و متروکه. برای همین هرچقدر هم پروژه‌ی زنده رو پاک‌سازی می‌کردیم، عدد توی SQL Editor کاربر «هیچ‌وقت تغییر نمی‌کرد» (چون اصلاً داشت جای دیگه‌ای رو می‌خوند) — این باعث چند دور بررسی اضافی و نتیجه‌گیری غلط («جاب هنوز کار نمی‌کنه») شد، درحالی‌که کد از یه‌جایی به بعد کاملاً درست کار می‌کرد. **عدد واقعی و درست پروژه‌ی زنده** (بعد از تأیید با آدرس URL درست `.../project/komqnapfqrtxxaytpcdt/sql/...`): فقط ۲ باکت اصلاً فایل دارن — `post-media` (۲۷۹ فایل، ۴۳۴.۶ مگابایت) و `newspaper-covers` (۱۴۴ فایل، ۲۳.۴ مگابایت)؛ جمعاً **~۴۵۸ مگابایت**، خیلی پایین‌تر از سقف ۱ گیگ. بقیه‌ی باکت‌ها (`magazines`, `archive-reports`, `archive-report-covers`, `magazine-covers`, `basirat-course-posters`, `channel-avatars`, `school-reports`) روی پروژه‌ی زنده **فعلاً هیچ فایلی ندارن** — چون طبق نکته‌ی عملیاتی ۱۸ فقط کانال‌ها/دسته‌بندی‌ها/مناطق دستی بعد از مهاجرت بازیابی شدن، نه محتوای این کتابخونه‌ها؛ اگه یه‌روز کاربر گفت این گالری‌ها خالی به‌نظر می‌رسن، دلیلش همینه، نه یه باگ جدید. **درس گرفته‌شده‌ی خیلی مهم برای هر بررسی Storage/دیتابیس آینده**: همیشه اول از کاربر بخواه که URL بالای مرورگرش رو موقع اجرای کوئری چک/تأیید کنه (باید دقیقاً `komqnapfqrtxxaytpcdt` رو توی مسیر داشته باشه) — مخصوصاً اگه عدد گزارش‌شده با هیچ اقدامی تغییر نکنه، این خودش قوی‌ترین نشونه‌ی «پروژه‌ی اشتباه»ه، قبل از اینکه فرض کنی کد/جاب خرابه.
    - **علت اصلی مغایرت مسیر همچنان به‌طور کامل تحلیل نشده**: چرا اون ۲۴۶ فایل اولیه اصلاً orphan شده بودن (یعنی چرا `media_storage_path`شون توی دیتابیس با هیچ ردیف زنده‌ای مچ نمی‌شد) صرفاً به همون باگ silent-fail قبلی (که مانع پاک‌سازی درست‌شون توسط cleanup_media.py می‌شد) نسبت داده شده؛ این توضیح با شواهد فعلی سازگاره و نیاز به کندوکاو بیشتر نداره، مگر اینکه orphan جدیدی بعد از این رفع دوباره دیده بشه.
...
- تحلیل احساسات یا طبقه‌بندی موضوعی *کامل* روی تک‌تک پست‌ها (چیزی که قبلاً برای n8n + Claude API برنامه‌ریزی شده بود) هنوز نیست. ⚠️ این با **سه** فیچر هوش مصنوعی واقعی و فعال موجود فرق داره: «ترجمه به فارسی» (تک‌پستی، با کلیک کاربر)، «تحلیل خودکار اخبار» تب «در یک نگاه» (دسته‌ای، روزی ۴ بار)، و «استخراج کلیدواژه‌ی پست‌ها» (هر پست، هر ۲ ساعت، پایه‌ی تب «پرونده ویژه») — این سومی هرچند AI پشت تک‌تک پست‌هاست، ولی فقط کلیدواژه استخراج می‌کنه، نه طبقه‌بندی موضوعی رسمی یا تحلیل احساسات؛ بخش‌های «هوش مصنوعی» بالاتر رو ببین.
- خروجی Excel هنوز کار نمی‌کنه (toast «به‌زودی» می‌ده؛ CSV/JSON واقعیه).
- ⚠️ **اورفلوی افقی موبایل روی ردیف تب‌های `.news-page-tabs` (بخش «اخبار و رویدادها»)**: با تست Playwright (viewport 390px) کشف شد که این ردیف (`display:flex` بدون `flex-wrap`/`overflow-x`، ۷ تب فعال + ۱ placeholder) عرضش از viewport موبایل رد می‌شه و باعث اسکرول افقی کل صفحه می‌شه. با مقایسه‌ی مستقیم با نسخه‌ی بدون تغییرات «پرونده ویژه» تأیید شد **pre-existing**‌ه (قبل از فعال‌شدن اون تب هم بوده، ربطی به فیچرهای اخیر نداره) — چون خارج از scope کارهای اخیر بود عمداً رفعش نشده؛ اگه یه‌روز کاربر گزارش داد یا خواست رفع بشه، راه‌حل احتمالی یا `overflow-x:auto` روی خودِ `.news-page-tabs` یا کوچیک‌کردن gap/فونت روی موبایله.
...
- **تصمیم Storage فعلاً موقتاً حل شده، نه قطعی**: به‌جای ارتقا به Supabase Pro یا مهاجرت به VPS شخصی، پروژه به یه Organization/پروژه‌ی تازه‌ی رایگان سوپابیس منتقل شد (نکته‌ی عملیاتی ۱۸ رو ببین) — این یه راه‌حل رایگان و سریع بود، نه تصمیم زیرساختی نهایی. اگه پروژه‌ی جدید هم به همون مشکل سهمیه بخوره (با توجه به نرخ جمع‌آوری روزانه‌ی فعلی، `RETENTION_DAYS=1` هم به‌زودی دوباره ممکنه محدودکننده بشه)، بحث «ارتقا به Pro در برابر VPS شخصی» باید دوباره و این‌بار به‌صورت جدی (نه به‌عنوان یه رفع فوری) باز بشه. ⚠️ **به‌روزرسانی ۱۷ شهریور ۱۴۰۵ (نکته‌ی عملیاتی ۲۱)**: بعد از رفع کامل باگ silent-fail حذف Storage و تأیید با آدرس پروژه‌ی درست (کاربر قبلش داشت SQL Editor پروژه‌ی قدیمی رو چک می‌کرد)، عدد واقعی Storage پروژه‌ی زنده فقط **~۴۵۸ مگابایته** (خیلی پایین‌تر از سقف ۱ گیگ) — فعلاً هیچ فوریتی برای ارتقا/مهاجرت وجود نداره؛ فقط اگه در آینده دوباره به سقف نزدیک شد، این بحث رو باز کن.
