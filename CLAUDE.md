...
4. **«چرا جریان»** (`.why-jarian-card`): یه کادر یکپارچه‌ی تمام‌عرض با ته‌رنگ گرادیانی هلویی (عیناً همون پس‌زمینه‌ی `.package-card`)، شامل سه ستون (`.why-jarian-grid` داخلش، بدون قاب جدا) با خط جداکننده‌ی عمودی بین ستون‌ها: «رصد لحظه‌ای» / «تحلیل تخصصی» / «دسترسی اختصاصی» — هرکدوم یه تگ عنوان کوتاه (`--accent-strong` رنگ، نه `--accent`، چون زمینه‌ی کارت روشنه نه بوم تیره)، یه خط کوتاه زیرش (`.why-jarian-bar`، با هاور کش میاد)، و یه پاراگراف توضیح. `margin-top: 160px`. ⚠️ نسخه‌ی قبلی‌ای که این سه ستون رو بدون قاب مستقیم روی بوم تیره نشون می‌داد (با یه خط اتصال موج‌دار زیرش) توسط کاربر رد شد — اگه جایی اشاره‌ای بهش دیدی منسوخه.
...
9. **ویجت «ضمیمه جریان»** (`.basirat-widget-block`، تنها ویجت عمومی زیر کارت هلویی، `margin: 220px 0 220px` — فاصله‌ی بالا/پایین عمداً برابر): دو بخش روی هم:
