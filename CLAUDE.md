...
7. **ردیف میان‌برهای زیربخش‌ها** (`.domain-shortcuts`، ۱۰ مورد): زیر توضیح سه‌خطی، لینک‌های pill-شکل به زیرتب‌های عرصه‌هایی که تب دارن (اخبار و رویدادها: ۷ تا با `goToNewsTab(tab)`، کنشگری مجازی: ۲ تا با `switchSection('posts')`، یادداشت‌ها: فقط ۱ تا — «چهره‌های سیاسی» با `switchSection('people')` مستقیم، چون از وقتی این بخش ساده شد دیگه زیرتب/تابع اختصاصی نداره — «برنامه مدارس» تب نداره پس ازش چیزی نیومده؛ «ضمیمه جریان» هم چون یکی از این چهار عرصه‌ی رصد نیست، اینجا لینک نداره). موارد فعال `<a onclick="...">` و موارد هنوز غیرفعال `<span class="domain-shortcut-disabled">...<span class="news-soon">به‌زودی</span></span>` (بدون گروه‌بندی با اسم عرصه — فقط اسم خودِ تب، کنار هم؛ چون بدون اسم عرصه دو مورد «آنالیز» تکراری می‌شدن، به «آنالیز اخبار»/«آنالیز کنشگری» تغییر نام دادن تا مشخص باشن). `goToNewsTab(tab)` ابتدا `switchSection` می‌زنه بعد تب داخلی رو عوض می‌کنه — تعریفش بلافاصله بعد از `switchSection()` توی کد اومده. ⚠️ **از قلم‌افتاده‌ی تازه‌کشف‌شده**: میان‌بر «آنالیز اخبار» هنوز `<span class="domain-shortcut-disabled">` با نشان «به‌زودی»ه، درحالی‌که خودِ تب («در یک نگاه»، پایین‌تر ببین) دیگه placeholder نیست — الان تب اول و پیش‌فرض «اخبار و رویدادها»ست. کسی هنوز درخواست نکرده این میان‌بر رو فعال کنن (`<a onclick="goToNewsTab('analytics')">`)، پس دست‌نخورده مونده — اگه کاربر ازش پرسید یا خواست هماهنگش کنی، همینه ماجرا.
